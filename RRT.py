...
class RRT():
    ...
    def change_goal_weights(self, nodes): # change the weight variable to np.inf for all nodes in the list
        for node in nodes:
            node.node_goal_weight = np.inf 
    ...
    def steer(self, nearest_node, sampled_node): 
        ...
        path = a_star_local.A_star_plan(time_limit=4)
...
if __name__  == "__main__":
    ...
    env.draw_obstacles()
    start = (0.0,0.0,np.array([0.0,0.0,0.0,0.0]))
    goal = (178.0,150.0,np.array([-90.0,-90.0,-90.0,-90.0]))
